...
This project modularizes the original `Unsupervised_Clustering_Solution.ipynb`
notebook into a maintainable, testable Python package with a Streamlit front
end, built for the CST2216 Machine Learning 2 term project.
...
├── data/
│   └── mall_customers.csv  # Training data
├── models/
│   └── cluster_model.pkl   # Trained KMeans model (generated by train.py)
├── src/
│   ├── data_loader.py      # Load + validate the dataset
│   ├── preprocessing.py    # Feature selection, k-range evaluation
│   ├── train.py             # Training pipeline (CLI + importable)
│   ├── predict.py           # Load model, assign new customers to a segment
...
| k | Elbow (WCSS) | Silhouette Score |
|---|---|---|
| 3-8 | computed for each | computed for each |

The pipeline automatically selects and persists the KMeans model with
the k that scores highest on silhouette, which matches the original
notebook's conclusion of k=5 on Annual Income and Spending Score.

## Author

Meet — CST2216 Machine Learning 2, Algonquin College.
